...
<!--   Arbre -->
    ...
    <script id="code">
        function init() {
            ...
            //1) connnexion a la base de donnée
            //			$connectionBdd = new PDO("mysql:host=$localhost;dbname=basemmi18db04", 'mmi18db04', 'qC04');
            //			$connectionBdd ->query("SET NAMES utf8");
            //2) préparer la requete sql pour récupérer tout les articles
            //			$marequete="SELECT * FROM grosdoit";
            //3) executer la requete
            //			$res = $connectionBdd->query($marequete);
            //			$resnom = $connectionBdd->query($marequete);

            $resultat= '[';


            foreach ($infosarbre as $valtab){
                $resultat.="{ key: ".$valtab->arbre_id.", n: '".$valtab->arbre_nom."', s: '".$valtab->arbre_sexe."'";

                if (!empty($valtab->arbre_id_mere)){
                    $resultat.=", m: ".$valtab->arbre_id_mere;
                }

                if (!empty($valtab->arbre_id_pere)){
                    $resultat.=", f: ".$valtab->arbre_id_pere;
                }

                if (!empty($valtab->arbre_id_femme)){
                    $resultat.=", ux: ".$valtab->arbre_id_femme;
                }

                if ($valtab->arbre_sexe=='M'){
                    $resultat.=", a: ['E']},";
                } else {
                    $resultat.=", a: ['H']},";
                }
            ...
            setupDiagram(myDiagram, <?php echo $resultat ?>,
